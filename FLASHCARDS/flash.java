...
import java.text.*;
   ...
   public class flash extends JFrame {
     ...
     gridConstraints.gridy = 0;
     gridConstraints.insets = new Insets(10, 0, 0, 0);getContentPane().add(triedTextField,gridConstraints);
     ...
     typePanel.setLayout(new GridBagLayout()); gridConstraints = new GridBagConstraints();

     gridConstraints.gridx = 0;
     gridConstraints.gridy = 3;
     gridConstraints.gridwidth = 2;
     gridConstraints.anchor = GridBagConstraints.NORTH;getContentPane().add(typePanel, gridConstraints); for (int i = 0; i < 4;i++) {
     ...
     typeCheckBox[3].setText("division");typeCheckBox[0].setSelected(true); 
     ...
     getContentPane().add(factorPanel, gridConstraints); int x = 2;
     int y = 0;
     for (int i = 0; i < 11; i++)
{
     ...
     for (int i = 0; i < 3; i++)
{
     ...
     timerRadioButton[1].setText("contador hacia arriba"); 
     timerRadioButton[2].setText("On-CountDown");	
     ...
     exitButton.addActionListener(new ActionListener() {
		...
		setBounds((int)((screenSize.width - getWidth())), (int) ((screenSize.height - getHeight())), getWidth(), getHeight());}
	...
	if (numberChecks == 0)
		...
		if (startButton.getText().equals("Start Practice")) {
			...
			timerScrollBar.setEnabled(false); 
			if(!timerRadioButton[01].isSelected()) {
			...
			if (numberTried > 0){
				...
				JOptionPane.showConfirmDialog(null, message, "Results", 
				...
				if (startButton.getText().equals("Start Practice")) return;
				...
				if (e.getKeyChar() >= '0' && e.getKeyChar() <= '9') {
					...
					if (digitNumber != numberDigits) {
						digitNumber++;
				problemLabel.setText(problemLabel.getText() + "?"); return;
					}
		else
		{
		numberTried++;
		...
		while (p == 0);
			...
			//random
			if (p == 4)
			return (myRandom.nextInt(9) + 1); else 
